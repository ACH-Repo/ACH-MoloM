...
(some formats loop forever on certain inputs) can be killed by the parent's
timeout instead of freezing the GUI. A worker *thread* wouldn't do: OpenBabel's
SWIG binding holds the GIL during the read, so a hung read blocks the whole
interpreter, not just one thread.

Vendored from ORCA Workbench (orca_workbench/core/_obabel_worker.py, MIT).

Not imported by the app at runtime; invoked as
    python -m molom.core._obabel_worker <fmt> <path>
...
def main(argv):
    ...
    try:
        from molom.core import io as _io
        _io._ensure_loggers_silenced()
        sym = _io._atomic_number_to_symbol
        try:
