...
class ExportOptions(object):
    ...
    def __init__(self, **kw):
        ...
        # C1: keyframe the scene clock's own frames. OFF by default,
        # because baking every frame is what makes a .blend large and a
        # still is the common export.
        self.animate = False
        self.unit_cell = True
...
# ------------------------------------------------------------------ script
def collect_animation(scene, style, options, times, seek, cell_of=None):
    # type: (object, object, ExportOptions, Sequence, object, object) -> dict
    """Per-frame positions for `collect`'s atom and bond lists.

    Roadmap C1. Every atom is already its own Blender object sharing one mesh
    as a linked duplicate, so an animation is per-object keyframes and needs
    no new geometry at all - which is why this is a collector rather than a
    second exporter.

    **EVERY RENDERED FRAME IS BAKED**, rather than keyframing the source
    frames and letting Blender interpolate. The docket proposed the second
    and it is wrong here: MoloM's own player uses `interpolate.rigid_lerp`,
    which splits the Kabsch rotation out and TURNS it, precisely because a
    plain lerp sends every atom along the chord and a rotating molecule
    visibly contracts at the half-way point (round 22). Blender's linear
    interpolation is that plain lerp. So keyframing only the source frames
    would make the render disagree with the viewport on exactly the case
    round 22 exists to fix - and round 37's rule is that an export which
    quietly disagrees with the screen is worse than none. The cost is file
    size, and it is stated rather than hidden (see `animation_summary`).

    `seek(t)` is supplied by the caller and must put the scene at scene-frame
    `t` exactly as playing it would - so the export cannot drift from the
    player by having its own idea of what a frame is. `times` is
    `animation.frame_times`, the same plan the image export renders.

    **CONNECTIVITY IS TAKEN FROM THE FIRST FRAME.** A bond is a Blender
    object, so the lists have to line up across frames; where a trajectory
    re-perceives different bonds partway along there is no correspondence to
    keyframe. The count is checked at every frame and any change is
    REPORTED - `bonding.FIXED_BONDS` already covers a baked vibration, and
    round 106's scan preview freezes them for the same reason.
    """
    frames_atoms, frames_bonds = [], []
    counts = set()
    for t in times:
        seek(t)
        atoms, bonds = [], []
        for obj in scene.visible_objects():
            ...
            bonds += b
            if options.unit_cell and cell_of is not None:
                ce, _cm = cell_edges(cell_of(obj), options.cell_radius)
                bonds += ce
        frames_atoms.append([a[1] for a in atoms])
        frames_bonds.append([[b[1], b[2]] for b in bonds])
        counts.add((len(atoms), len(bonds)))
    notes = []
    if len(counts) > 1:
        notes.append(
            "the connectivity changes during the animation, so the bonds of "
            "the first frame are the ones keyframed")
        n_atoms = min(c[0] for c in counts)
        n_bonds = min(c[1] for c in counts)
        frames_atoms = [f[:n_atoms] for f in frames_atoms]
        frames_bonds = [f[:n_bonds] for f in frames_bonds]
    return {"atoms": frames_atoms, "bonds": frames_bonds, "notes": notes}


def animation_summary(anim):
    # type: (Optional[dict]) -> str
    """What baking every frame costs, said out loud.

    A keyframe per atom per frame is the honest way to make a render match
    the viewport, and it is also the thing that makes a .blend large - so the
    number goes in the export's own summary rather than being discovered
    later.
    """
    if not anim or not anim.get("atoms"):
        return ""
    n_frames = len(anim["atoms"])
    n_atoms = len(anim["atoms"][0]) if anim["atoms"] else 0
    n_bonds = len(anim["bonds"][0]) if anim["bonds"] else 0
    keys = n_frames * (n_atoms + n_bonds)
    return "{} frames baked, {} keyframed objects, {} keyframes".format(
        n_frames, n_atoms + n_bonds, keys)
...
# One key per RENDERED frame for every atom and every bond segment, rather
# than the source frames with Blender interpolating between them: MoloM's
# own player turns the rigid part of the motion as a rotation, and Blender's
# linear interpolation would take the chord instead - the contraction round
# 22 exists to remove. Baking is what makes the render match the viewport.
ANIMATION = {animation}
'''
...
def bond_matrix(start, end, radius):
    """A unit cylinder placed along `start`->`end` at `radius`.

    One function, because the bond is BUILT with it and then keyframed with
    it - two copies of this arithmetic is how a still and an animation come
    to disagree about where a stick is.
    """
    ...
    return (Matrix.Translation((a + b) / 2.0) @ rot
            @ Matrix.Diagonal((radius, radius, length, 1.0)))


def build_animation(atom_objects, bond_objects):
    """Keyframe every atom and bond, one key per rendered frame.

    Baked rather than interpolated on purpose - see `collect_animation` in
    MoloM. The interpolation mode is still set to LINEAR, because although a
    key on every frame is never sampled BETWEEN keys by an ordinary render,
    motion blur and any change of frame rate do sample between them, and
    Bezier easing there would overshoot.
    """
    if not ANIMATION or not ANIMATION.get("atoms"):
        return 0
    scene = bpy.context.scene
    first = int(ANIMATION.get("frame_start", 1))
    positions = ANIMATION["atoms"]
    segments = ANIMATION.get("bonds") or []
    scene.frame_start = first
    scene.frame_end = first + len(positions) - 1
    fps = int(ANIMATION.get("fps", 0) or 0)
    if fps > 0:
        scene.render.fps = fps
    for k, frame_atoms in enumerate(positions):
        frame = first + k
        for ob, p in zip(atom_objects, frame_atoms):
            ob.location = Vector(p)
            ob.keyframe_insert("location", frame=frame)
        if k < len(segments):
            for (ob, radius), seg in zip(bond_objects, segments[k]):
                ob.matrix_world = bond_matrix(seg[0], seg[1], radius)
                ob.keyframe_insert("location", frame=frame)
                ob.keyframe_insert("rotation_quaternion", frame=frame)
                ob.keyframe_insert("scale", frame=frame)
    for ob in list(atom_objects) + [b[0] for b in bond_objects]:
...
def main():
    ...
    for n, (mat_name, position, radius) in enumerate(ATOMS):
        ...
        add_subsurf(ob)
        atom_objects.append(ob)

    bond_coll = sub_collection(root, "bonds")
    bond_objects = []
    for n, (mat_name, start, end, radius) in enumerate(BONDS):
        ob = new_object("bond.{0}".format(n), cylinder, mats.get(mat_name),
                        bond_matrix(start, end, radius), bond_coll)
        # QUATERNION, because a bond's rotation is keyframed and Euler
        # channels can flip through gimbal lock between two frames that are
        # a few degrees apart - which reads as one stick spinning while its
        # neighbours do not.
        ob.rotation_mode = "QUATERNION"
        ob.matrix_world = bond_matrix(start, end, radius)
        bond_objects.append((ob, radius))

    n_keys = build_animation(atom_objects, bond_objects)
    ...
    print("MoloM: {0} atoms, {1} bond segments, {2} polyhedra, {3} materials, "
          "{4} lights, {5} camera(s), {6} animated frame(s), engine {7}".format(
              len(ATOMS), len(BONDS), len(POLYHEDRA), len(MATERIALS),
              n_lights, len(SAVED_CAMERAS), n_keys, engine))
...
def build_script(data, options, title="scene", version="", basename="",
    ...
    return ascii_only(_HEADER.format(
        ...
        animation=_pretty(data.get("animation") or {}),
    ) + _BODY)
...
def summarise(data):
    ...
    out += ", {} materials, {} lights".format(len(data["materials"]),
                                                len(data["lights"]))
    extra = animation_summary(data.get("animation"))
